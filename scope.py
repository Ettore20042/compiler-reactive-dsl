"""
Modulo: scope.py
Gestisce l analisi degli scopi: dichiarazioni, visibilita' variabili, registrazione funzioni.
"""
...
class ScopeAnalyzer:
    ...
    def visit_VarDecl(self, node):
        ...
        # Nota: il type checking è demandato a TypeChecker
        self.symbol_table[node.name] = node.type

    def visit_SetStmt(self, node):
        """Verifica che la variabile esista (non verifica il tipo, fatto in TypeChecker)."""
        if node.name not in self.symbol_table:
            raise Exception(f"Errore: variabile '{node.name}' non dichiarata.")

    def visit_CallStmt(self, node):
        """Verifica che la funzione esista (non verifica i tipi degli argomenti, fatto in TypeChecker)."""
        if node.name not in self.functions:
            raise Exception(f"Errore: funzione '{node.name}' non dichiarata.")

    def visit_ReturnStmt(self, node):
        """Verifica che il return sia dentro una funzione (non verifica il tipo, fatto in TypeChecker)."""
